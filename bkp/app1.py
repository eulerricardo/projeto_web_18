...
import os
import logging
from flask import Flask, session, render_template, jsonify, request, send_from_directory, redirect, url_for
import json
...
# Configuração para a chave secreta da sessão
app.secret_key = os.urandom(24)


# Definindo a senha diretamente no código (não recomendado para produção)
USUARIO = 'euler.ricardo@gmail.com'
SENHA = 'Jope1234'
...
def login():
    """Página de login onde o usuário entra com suas credenciais."""
    if request.method == 'POST':
        ...
        if usuario_input == USUARIO and senha_input == SENHA:
            session['usuario'] = usuario_input  # Guarda a informação na sessão
            return redirect(url_for('index'))  # Redireciona para a página inicial
        else:
            # Caso as credenciais sejam incorretas
            return render_template('login.html', erro="Usuário ou senha inválidos!")
    
    # Se for um GET, apenas exibe a página de login
